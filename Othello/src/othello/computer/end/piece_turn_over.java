...
public class piece_turn_over {
  ...
  public static boolean main(int cx,int cy,int t) {
    ...
    x=cx;
    y=cy;
    if(thinking.p[10*x+y] == 0){//ズルはいけません(笑
      if (turn_x(8, 1, t)) //右
        check = true;
      if (turn_x(0, -1, t)) //左
        check = true;
  ...
  static boolean turn_x(int xn,int xh,int t){
    ...
    if(x != xn){
      for(int i=1;i<=(xh*x*-1)+xn;i++){
        if (thinking.p[(x+(xh*i))*10+y] == t) {//自分の駒が発見されればBreak;
          tmp = i-1;
          c=i;
          check = true;//場合にもよるが，とりあえずひっくり返せる．
          break;
        }else if(thinking.p[(x+(xh*i))*10+y] == 0){
          //自分の駒が発見されずに
          //空白を検知したらひっくり返せない
          check = false;
          break;
  ...
  static boolean turn_y(int yn,int yh,int t){
    ...
    if(y != yn){
      for(int i=1;i<=(yh*y*-1)+yn;i++){
        if (thinking.p[(x*10+y+(yh*i))] == t) {//自分の駒が発見されればBreak;
          tmp = i-1;
          c=i;
          check = true;//場合にもよるが，とりあえずひっくり返せる．
          break;
        }else if(thinking.p[x*10+y+(yh*i)] == 0){
          //自分の駒が発見されずに
          //空白を検知したらひっくり返せない
          check = false;
          break;
  ...
  static boolean turn_naname(int xn,int yn,int xh,int yh,int t){
    ...
    if(x != xn && y != yn){
      for(int i=1;i<=Math.min((xh*x*-1)+xn,(yh*y*-1)+yn);i++){
        if (thinking.p[(x+(xh*i))*10+(y+(yh*i))] == t) {//自分の駒が発見されればBreak;
          tmp = i-1;
          c=i;
          check = true;//場合にもよるが，とりあえずひっくり返せる．
          break;
        }else if(thinking.p[(x+(xh*i))*10+(y+(yh*i))] == 0){
          //自分の駒が発見されずに
          //空白を検知したらひっくり返せない
          check = false;
          break;
