package othello.piece_turn_over;
import othello.*;
import java.awt.*;
...
public class piece_turn_over {
  static int p[];
  static int x;
  static int y;
  public static boolean main(int cx,int cy,int t,ImageIcon t_img) {
    boolean check = false;
    p=MainFrame.p;
    x=cx;
    y=cy;
    if(p[10*x+y] == 0){//ズルはいけません(笑
      if (turn_x(8, 1, t, t_img)) //右
        check = true;
      if (turn_x(0, -1, t, t_img)) //左
        check = true;

      if (turn_y(8, 1, t, t_img)) //下
        check = true;
      if (turn_y(0, -1, t, t_img)) //上
        check = true;

      if (turn_naname(8, 8, 1, 1, t, t_img)) ///右下
        check = true;
      if (turn_naname(8, 0, 1, -1, t, t_img)) //右上
        check = true;
      if (turn_naname(0, 8, -1, 1, t, t_img)) //左下
        check = true;
      if (turn_naname(0, 0, -1, -1, t, t_img)) //左上
        check = true;
    }
    return check;
  }
  static boolean turn_x(int xn,int xh,int t,ImageIcon t_img){
    /*
    ...
    if(check){//ひっくり返せるかチェックして，OKならひっくり返すッ！
      for(int i=0;i<=c-1;i++){
        p[(x+(xh*i))*10+y]=t;
        MainFrame.piece[(x+(xh*i))*10+y].setIcon(t_img);
      }
    }
    return check;

  }
  static boolean turn_y(int yn,int yh,int t,ImageIcon t_img){
    /*
    ...
    if(check){//ひっくり返せるかチェックして，OKならひっくり返すッ！
      for(int i=0;i<=c-1;i++){
        p[x*10+y+(yh*i)]=t;
        MainFrame.piece[x*10+y+(yh*i)].setIcon(t_img);
      }
    }
    return check;
  }

  static boolean turn_naname(int xn,int yn,int xh,int yh,int t,ImageIcon t_img){
    /*
    ...
    if(check){//ひっくり返せるかチェックして，OKならひっくり返すッ！
      for(int i=0;i<=c-1;i++){
        p[(x+(xh*i))*10+(y+(yh*i))]=t;
        MainFrame.piece[(x+(xh*i))*10+(y+(yh*i))].setIcon(t_img);
      }
    }
    return check;
  }
