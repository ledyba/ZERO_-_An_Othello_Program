...
public class tujo {
  ...
  static int[][] hyoka_value = {
      ...
      {-11,-16, -1, -3, -3, -1,-16,-11,},
      { 45,-11,  4, -1, -1,  4,-11, 45,}};
  static int[][] hyoka_omosa = {//評価値の重さ[深さ][値の種類]
      //ひとつづき，確定石，場所，着手可能な場所，ウイング，開放度 ,
      //ライン3*3 , ライン4*4 , ライン5*5 , ライン6*6 , ライン7*7 ,ライン8*8 ,
...
public void start(int t , int hukasa) {
      ...
      System.out.println("現局面の評価："+hyo(my_turn , Frame.isi-3));
      //for(int i=1;i<=max;i++){
     //   kmax = i;
      //  for(int j=0;j<=i;j++){
      //    cut[j]=0;
      ...
      for(int i=1;i<=saishu_max;i++){
        ...
        System.out.println("深さ："+i+"　最善手：" + saizente[1][0] + "," + saizente[1][1]
  ...
  public static int[][] saiki(int hukasa  ,int max, int turn ,int isi , int alpha ,int beta){
    ...
    int scout[][];
    int scout_count = 0;
    int tmp[][] = new int[max+1][3];//探索時に使用するそのての行き先を記した配列
    int saizen_kouho[][] = new int[max+1][3];//最善候補と思われる手を記し，上のノードに渡す
    int w_isi=0;
    int b_isi=0;
    int tyakushu = 0;
    ...
    for (int i = 1; i <= 8; i++) {
      for (int j = 1; j <= 8; j++) {
        if(rule.rule(i, j, turn)){//ひっくり返し＋ルールチェック
          pass_check=true;
    ...
    for(int i=0;i<tyakushu;i++){//反復進化のために，仮の最善手のポイントは上げておく
...
for(int i=0;i<tyakushu;i++){
    ...
    for (int i = 0; i < tyakushu; i++) {
      ...
      if(hikkuri.rule(scout[i][0], scout[i][1], turn)){//ひっくり返し＋ルールチェック
        ...
        if(hukasa == 1 && max == saishu_max){
          System.out.println("座標:("+scout[i][0]+","+scout[i][1]+")　評価値："+tmp[max][2]);
        }
        if( turn == my_turn ){
  ...
  public static int kaihoudo(){//めんどくさいので，相手の開放度で一番小さいものを探す
    int hyoka=0;
    int count=0;
    int teki_saitei=9;
    int my_saitei=9;
