...
//scout法じゃないけどさ．
//なんでまちがえたんだろう
//NegaMaxを試すも，なぜかすべての手の評価値が同じような評価値になる．Why？
/**
 * <p>タイトル: ZERO</p>
 * <p>説明: </p>
...
public class tujo {
  ...
  static double[][] hyoka_omosa = {//評価値の重さ[深さ][値の種類]
      ...
      { 1, 10,  1,  1, 10,  1,  3,  3,  3,  3,  3,  3, 0.25,},
  };
...
*/
  ...
  public static double saiki(int hukasa  ,int end, int turn ,int isi ,double alpha, double beta,int kaihoudo){
    int t=0;
    ...
    if(tyakushu == 0){//パスチェックをしらべて，そうだったらパス
      return -1 * saiki(hukasa + 1, end, t , isi +1 , -beta , -alpha,kaihoudo);
    }
    if((isi > 64) || (w_isi == 0 || b_isi == 0)){
      if(w_isi == 0 && my_turn == 0){
        return 9999;
      }else if(b_isi == 0 && turn == 0){
        return -9999;
      }else if(w_isi == 0 && turn == 1){
        return -9999;
      }else if(b_isi == 0 && turn == 1){
        return 9999;
      }else{
        if(turn == 1){
          if (w_isi > b_isi) {
    ...
    for (int i = 1; i <= 8; i++) {
      for (int j = 1; j <= 8; j++) {
        if (rule.rule(i, j, turn)) { //ルールチェック
          ...
          tmp = -1 *
              (saiki(hukasa + 1, end, t, isi + 1 , -beta , -Math.max(alpha , max),kaihoudo+kaihoudo(tmp_b,turn)));
